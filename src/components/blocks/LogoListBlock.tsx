                ...
                <div className="flex items-center gap-6 flex-wrap">
                    ...
                    {technologies.map(tech => (
                        <div
                            ...
                            className={`relative group flex items-center justify-center cursor-pointer transition-opacity ${selectedTechs.length > 0 && !selectedTechs.includes(tech.id)
                                    ? 'opacity-30'
                                    : 'opacity-100'
                                }`}
                        >
                            {tech.logo_url ? (
                                <Image
                                    ...
                                    width={32}
                                    height={32}
                                    className={`object-contain transition-all ${selectedTechs.includes(tech.id)
                                            ? 'grayscale-0'
                                            : 'grayscale hover:grayscale-0'
                                        }`}
                                />
                            ) : (
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium transition-colors ${selectedTechs.includes(tech.id)
